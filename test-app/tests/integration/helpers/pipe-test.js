...
module('Integration | Helper | {{pipe}}', function(hooks) {
  ...
  test('it pipes actions', async function(assert) {
    ...
    await render(hbs`
      <p>{{this.value}}</p>
      <button type="button" {{action (pipe (action "add") (action "square") (action "squareRoot")) 2 4}}>
        Calculate
      </button>
    `);
  ...
  test('it handles promises', async function(assert) {
    ...
    await render(hbs`
      <p>{{this.value}}</p>
      <button type="button" {{action (pipe (action "add") (action "square") (action "resolvify") (action "squareRoot")) 2 4}}>
        Calculate
      </button>
    `);
